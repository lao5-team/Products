...
public class PhotoSelectorActivity extends Activity implements
	...
	private void ok() {
		if (selected.isEmpty()) {
			...
			data.putExtra("photos", new Gson().toJson(selected));
	...
	public void onItemClick(int position) {
		...
		bundle.putString("album", (String)tvAlbum.getTag());
		CommonUtils.launchActivity(this, PhotoPreviewActivity.class, bundle);
	}
		...
		public void onPhotoLoaded(List<PhotoModel> photos) {
			for (PhotoModel model : photos) {
				if (selected.contains(model)) {
					...
					model.setIndex(selected.indexOf(model) + 1);
				}
			}
